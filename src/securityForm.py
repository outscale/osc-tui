...
class SecurityForm(npyscreen. FormBaseNew):
    ...
    def create(self):
        ...
        y, _ = self.useable_space()
        self.add(SecurityGrid, name='Security', value=0,
                 additional_y_offset=2, additional_x_offset=2,
                 max_height=int(y/2-2), column_width=25, select_whole_line=True,
                 on_selection=on_selection, scroll_exit=True)
        lbl_status = self.add_widget(npyscreen.Textfield, rely=self.draw_line_at + 2, value="No security group selected",
                                     editable=False)
