...
def add_inspector(form):
    ...
    terminate = form.add_widget(ButtonPress, name="TERMINATE")
    cp_ip = form.add_widget(ButtonPress, name="COPY IP")
    exit = form.add_widget(ButtonPress, name="EXIT")
    exit.whenPressed = main.exit
    i = Inspector(a, run_stop, restart, force_stop, terminate,cp_ip)
    return i

class Inspector():
    def __init__(self, name_label, run_stop, restart,
            force_stop, terminate, cp_ip):
        self.copy_ip = cp_ip
...
class VmInspector(BoxTitle):
    ...
    def set_value(self, values):
        self.name = values[1]
        #self.values = list().append('RENAME STOP RUN RESTART'.split())
        #self.values = values
        self.update()
